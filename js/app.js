(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            new swiper_core_Swiper(".trustme__slider", {
            ...
            new swiper_core_Swiper(".full-cycle__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
                ...
                speed: 800,
                navigation: {
                    prevEl: ".full-cycle-swiper-button-prev",
                    nextEl: ".full-cycle-swiper-button-next"
                },
                breakpoints: {
                    375: {
                        slidesPerView: 1.26,
            ...
            function initVideoSliders() {
                if (window.innerWidth > 768) {
                    ...
                    videoMain = new swiper_core_Swiper(".video-comments__slider", {
                        modules: [ Thumb ],
                        slidesPerView: 1,
                        spaceBetween: 0,
                        thumbs: {
    ...
    da.init();
    document.querySelector(".language-select__toggle").addEventListener("click", function() {
        document.querySelector(".language-select").classList.toggle("open");
    });
    document.querySelectorAll(".language-select__dropdown li").forEach(item => {
        item.addEventListener("click", function() {
            const short = this.getAttribute("data-short");
            document.querySelector(".language-short").textContent = short;
            document.querySelector(".language-select").classList.remove("open");
        });
    });
    document.addEventListener("click", function(e) {
        const box = document.querySelector(".language-select");
        if (!box.contains(e.target)) box.classList.remove("open");
    });
    document.querySelectorAll(".menu__item--has-children").forEach(item => {
        const btn = item.querySelector(".menu__link--dropdown");
    ...
    script_form.addEventListener("submit", function(e) {
        ...
        if (numbersOnly.length !== 12) {
            e.preventDefault();
            alert("Номер телефону має містити рівно 12 цифр після +38.");
